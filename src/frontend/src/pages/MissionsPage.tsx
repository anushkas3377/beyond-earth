...
function calcResult(
  ...
  // uranus/neptune: only advanced with high fuel has a chance
  if (dest === "uranus" || dest === "neptune") return "fail";
  return "fail";
}
...
function getFailReason(
  ...
  if (
    rocket === "heavy" &&
    (dest === "saturn" || dest === "uranus" || dest === "neptune")
  )
    return "Heavy Rocket fuel efficiency is too low for this distance.";
  if ((dest === "uranus" || dest === "neptune") && rocket === "advanced")
...
function getMissionStats(dest: DestinationId, fuel: number) {
  ...
  return {
    distance: d.distance,
    fuelEfficiency: fl,
    difficulty: difficultyMap[d.difficulty],
  };
}
...
export default function MissionsPage() {
  ...
  function handleLaunch() {
    ...
    const res = calcResult(selectedRocket, selectedDest, fuelLevel);
  ...
  return (
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-neon-cyan mb-3">
            Mission Control
          </p>
          <h1
...
function StepIndicator({ current }: { current: 1 | 2 | 3 }) {
  const steps = [
    { n: 1, label: "Choose Rocket" },
    { n: 2, label: "Select Destination" },
    { n: 3, label: "Configure Fuel" },
  ];
  return (
    <div
      className="flex items-center justify-center gap-0 mb-10"
      data-ocid="missions.step_indicator"
    >
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.label} className="flex items-center">
            ...
            {i < steps.length - 1 && (
              <div
                className="w-16 sm:w-24 h-px mx-2 sm:mx-3 mb-4 sm:mb-0"
                style={{
                  background: isDone
                    ? "linear-gradient(90deg, #6c63ff, rgba(0,229,255,0.3))"
...
// ─── Step 1: Choose Rocket
function StepOne({
  selected,
  onSelect,
  onNext,
}: {
  selected: RocketId | null;
  onSelect: (id: RocketId) => void;
  onNext: () => void;
}) {
  return (
    <motion.div
      ...
      data-ocid="missions.step1.section"
    >
      <h2
        className="text-center text-2xl sm:text-3xl font-bold text-neon-cyan mb-8"
        style={{ fontFamily: "Orbitron, sans-serif" }}
      >
        ...
        {ROCKETS.map((rocket) => {
          ...
          return (
            <button
              key={rocket.id}
              type="button"
              data-ocid={`missions.rocket.${rocket.id}`}
              ...
              style={{
                ...
                boxShadow: isSelected
                  ? "0 0 28px rgba(0,229,255,0.4), inset 0 0 20px rgba(0,229,255,0.06)"
                  : undefined,
                transform: isSelected ? "scale(1.03)" : undefined,
              }}
            >
              <div
                className="mb-4 flex items-center justify-center w-16 h-16 rounded-xl"
                style={{
                  background: `${rocket.iconColor}14`,
                  border: `1px solid ${rocket.iconColor}35`,
                  boxShadow: `0 0 18px ${rocket.iconColor}25`,
                }}
              >
                <rocket.icon
                  size={32}
                  style={{ color: rocket.iconColor }}
                  strokeWidth={1.5}
                />
              </div>
              <h3
      ...
      <div className="flex justify-center">
        <button
          type="button"
          data-ocid="missions.step1.next_button"
          className="cta-primary px-8 py-3 flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
          disabled={!selected}
          onClick={onNext}
        >
...
function StepTwo({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -40 }}
      transition={{ duration: 0.4 }}
      data-ocid="missions.step2.section"
    >
      <h2
        className="text-center text-2xl sm:text-3xl font-bold text-neon-cyan mb-8"
        style={{ fontFamily: "Orbitron, sans-serif" }}
      ...
      {activeDest && (
        <div
          className="pointer-events-none fixed inset-0 transition-smooth"
          style={{
            background: `radial-gradient(ellipse at 70% 40%, ${activeDest.glow} 0%, transparent 60%)`,
            zIndex: 0,
          }}
        />
      )}
      ...
      <div className="relative z-10 flex justify-center gap-3">
        <button
          type="button"
          data-ocid="missions.step2.back_button"
        ...
        <button
          type="button"
          data-ocid="missions.step2.next_button"
          className="cta-primary px-8 py-3 flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
          disabled={!selected}
          onClick={onNext}
        >
...
function StepThree({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -40 }}
      transition={{ duration: 0.4 }}
      data-ocid="missions.step3.section"
    ...
    >
      ...
      <div className="glass-card p-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <span
            ...
            style={{
              fontFamily: "Orbitron, sans-serif",
              color: fuelColor,
              textShadow: `0 0 12px ${fuelGlow}`,
            }}
          >
            {fuelLevel}% — {fuelLabel} Fuel
          </span>
        </div>
        ...
        <div className="relative mb-4">
          <input
            ...
            max={100}
            value={fuelLevel}
            data-ocid="missions.fuel.slider"
            onChange={(e) => onFuelChange(Number(e.target.value))}
            className="w-full h-2 rounded-full appearance-none cursor-pointer"
            style={{
              background: `linear-gradient(90deg, ${fuelColor} 0%, ${fuelColor} ${fuelLevel}%, rgba(255,255,255,0.1) ${fuelLevel}%, rgba(255,255,255,0.1) 100%)`,
              boxShadow: `0 0 8px ${fuelGlow}`,
            }}
          />
        </div>

        <div className="flex justify-between text-xs text-muted-foreground mb-8">
          <span style={{ color: fuelLabel === "Low" ? fuelColor : undefined }}>
            Low (0–33)
          </span>
          <span
            style={{ color: fuelLabel === "Optimal" ? fuelColor : undefined }}
          >
            Optimal (34–66)
          </span>
          <span style={{ color: fuelLabel === "High" ? fuelColor : undefined }}>
            High (67–100)
          </span>
        </div>

        <div
          className="rounded-lg h-6 overflow-hidden mb-8"
          style={{
        ...
        >
          <div
            ...
            style={{
              width: `${fuelLevel}%`,
              background: `linear-gradient(90deg, ${fuelColor}80, ${fuelColor})`,
              boxShadow: `0 0 10px ${fuelGlow}`,
            }}
          />
        </div>
        ...
        {fuelLabel === "Low" && (
          <motion.p
            ...
            style={{ color: "#ff4d4d" }}
          >
            ⚠️ Low fuel may result in mission failure
          </motion.p>
        )}
      </div>
      ...
      <div className="flex justify-center gap-3 mt-6">
        <button
          type="button"
          data-ocid="missions.step3.back_button"
          className="cta-secondary px-6 py-3"
          onClick={onBack}
        >
          Back
        ...
        <button
          type="button"
          data-ocid="missions.step3.launch_button"
...
function ResultScreen({
  ...
  rocket: RocketId;
  fuelLevel: number;
  stats: { distance: string; fuelEfficiency: string; difficulty: string };
  failReason: string;
  onReset: () => void;
}) {
  ...
  return (
    <motion.div
      ...
      data-ocid="missions.result.section"
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow: `0 0 50px ${glowColor}, 0 0 100px ${glowColor.replace("0.25", "0.1")}`,
        }}
      >
          ...
          <img
            ...
            style={{
              filter: isSuccess
                ...
                : "brightness(0.5) saturate(0.4) hue-rotate(340deg)",
            }}
          />
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {isSuccess ? (
              <CheckCircle2
                ...
                style={{
                  color: "#00ff9c",
                  filter: "drop-shadow(0 0 20px #00ff9c)",
                }}
              />
            ) : (
              <XCircle
        ...
        <div className="p-8">
          <h2
            className="text-center text-3xl sm:text-4xl font-bold mb-3"
            style={{
              fontFamily: "Orbitron, sans-serif",
              color: accentColor,
          ...
          <p className="text-center text-muted-foreground mb-8">
            {isSuccess
              ? `Your crew has safely reached ${destination.name}!`
              : `Your mission encountered critical failure before reaching ${destination.name}.`}
          </p>
          ...
          {isSuccess ? (
            <div className="grid grid-cols-3 gap-3 mb-8">
              ...
              <StatCard
                ...
                color={DIFFICULTY_COLOR[destination.difficulty] ?? accentColor}
              />
            </div>
          ) : (
            <div
            ...
            >
              <p className="text-sm" style={{ color: "#ff8080" }}>
                <span className="font-bold">Failure reason:</span> {failReason}
              </p>
            </div>
          )}
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <button
              type="button"
              data-ocid="missions.result.try_again_button"
            ...
            <Link
              to="/planet/$name"
              params={{ name: destination.id }}
              data-ocid="missions.result.explore_destination_link"
              className="cta-secondary flex items-center gap-2 px-6 py-3"
            >
              <Globe className="w-4 h-4" /> Explore {destination.name}
            </Link>
            <Link
              to="/lab"
              data-ocid="missions.result.test_in_lab_link"
              className="cta-secondary flex items-center gap-2 px-6 py-3"
            >
